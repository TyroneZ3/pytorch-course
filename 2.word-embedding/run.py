...
class WordEmbeddingDataset(Dataset):
    ...
    def __getitem__(self, idx):
        center_word = self.text_encoded[idx]    #[batch_size]
        pos_indices = list(range(idx-3, idx)) + list(range(idx, idx+3))
        pos_indices = [i % len(self.text_encoded) for i in pos_indices]
        pos_words = [self.text_encoded[i] for i in pos_indices] #[batch_size, 6]
...
class EmbeddingModel(nn.Module):
    ...
    def forward(self, input_labels, pos_labels, neg_labels):
        ...
        log_pos = torch.bmm(pos_embeddings, input_embeddings.unsqueeze(2))  #[batch_size, 6]
        log_neg = torch.bmm(neg_embeddings, input_embeddings.unsqueeze(2))  #[batch_size, 6*100]
        
        log_pos = nn.LogSigmoid(log_pos)
        log_neg = nn.LogSigmoid(log_neg)
